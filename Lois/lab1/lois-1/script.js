//@author Шамрук Евгений
//@group 721701
//@year 2020
//@subject ЛОИС
//@title Лабораторная работа N1
/*@task D: Проверить является ли строка формулой СДНФ.*/

//Включение строгого стандарта написания js скриптов
"use strict";

//Опредеоение константных переменных
    ...
    if (expression != "") {
        if (expression.length == 1) {
            ...
            if (reg.test(expression)) {
                document.getElementById(ANSWER_1_ID).innerHTML = 'Введенная формула является СДНФ';
            } else {
                document.getElementById(ANSWER_1_ID).innerHTML = 'Введенная формула не является СДНФ';
            }
        } else {
            ...
            if (isFormula) {
                ...
                if (expression.match(/((!)[(])/)) {
                    document.getElementById(ANSWER_1_ID).innerHTML = 'Введенная формула не является СДНФ, так как содержит двойные или групповые отрицания';
                    return;
                }
                if (boolResult) {
                    document.getElementById(ANSWER_1_ID).innerHTML = 'Введенная формула является СДНФ';
